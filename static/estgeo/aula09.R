...
# Visao mais realistica aos dados

## Resumo descritivo
summary(dados_geotecnia)

## Detalhamento
dados_geotecnia |>
  group_by(Profundidade) |>
  summarise(
    Media_Umid = mean(Umidade),
    Mediana_Umid = median(Umidade),
    CV_Umid = Umidade |> new_leem(variable = 2) |> cv(grouped = FALSE)
  )
