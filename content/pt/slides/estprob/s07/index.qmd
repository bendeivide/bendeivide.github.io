...
<tbody>
  ...
  <tr>
    ...
    <td class="tg-c3ow">$A = 28,0874035 - 0,2399639 = 15,29391~und.$</td>
  </tr>
</tbody>
</table>
...
\begin{align}
  SQ & = \displaystyle\sum_{i = 1}^{n}X_i^2 - \frac{1}{n} \sum_{i = 1}^{n}X_i^2,
\end{align}
...
\begin{align}\label{eq:varamost3}
    S^2 & = \frac{\displaystyle\sum_{i = 1}^{n}X_i^2 - \frac{1}{n} \sum_{i = 1}^{n}X_i^2}{n - 1}.
\end{align}
...
\begin{align*}
    S^2_{\textrm{gA}} & = \frac{10,037492^2 + \ldots + 9,487043^2 - 1 / 10 \times \left(10,037492 +  \ldots + 9,487043 \right)^2}{10 - 1}\\
...
\begin{align*}
    S^2_{\textrm{gB}} & = \frac{10,074985^2 + \ldots + 18,787223^2 - 1 / 10 \times \left(10,074985 +  \ldots + 18,787223 \right)^2}{10 - 1}\\
...
\begin{align*}
    S^2_{\textrm{gC}} & = \frac{10,1124770^2 + \ldots + 28,0874035^2 - 1 / 10 \times \left(10,1124770 +  \ldots + 28,0874035 \right)^2}{10 - 1}\\
                    & = \frac{1373,903 - 8133.67 / 10}{9}\\
                    & = 62,28176~und^2
  \end{align*}
...
- isso significa que se tivermos usando uma variável na escala de metros, a dispersão dada pela variância estará na escala de área;
...
**Definição (Desvio padrão):** Seja uma amostra $X_1$, $X_2$, \ldots, $X_n$, de uma população $X_1$, $X_2$, \ldots, $X_N$, de tamanhos $n$ e $N$, respectivamente, com parâmetro  $\mu = \sum_{i = 1}^{N} X_i / N$ e seu estimador $\bar{X} = \sum_{i = 1}^{n} X_i / n$, então o desvio padrão é definido por:
...
## Coeficiente de variação {background-image="cap02.png"}

<font size="+3">

**Definição (Coef. de variação):** Seja uma amostra $X_1$, $X_2$, $\ldots$, $X_n$, de uma população $X_1$, $X_2$, $\ldots$, $X_N$, de tamanhos $n$ e $N$, respectivamente, com parâmetros  $\mu = \sum_{i = 1}^{N} X_i / N$ e $\sigma^2 = \sum_{i = 1}^{N}(X_i - \mu)^2 / (N - 1)$ e seus estimadores $\bar{X} = \sum_{i = 1}^{n} X_i / n$ e $S = \sqrt{\sum_{i = 1}^{n}(X_i - \bar{X})^2 / (n - 1)}$, respectivamente. Então o coeficiente de variação é definido por:
...
- Erro padrão da média amostral para o grupo gC:
  \begin{align*}
   S_{\bar{X}_{\textrm{gC}}} & = 7,89188 / \sqrt{10} = 2,495632~und.
  \end{align*}
</font>
