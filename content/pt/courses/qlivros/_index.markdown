---
title: 💻 Quarto Livros
linkTitle: QLivros
summary: Um curso ministrado no Youtube para interessados a autores independentes que buscam desenvolver seus próprios livros.
date: "2026-01-16 09:53:31"
type: book
categories: ["R", "Livro", "Programação"]
tags: ["Curso", "R", "Livro", "Programação", "Digital", "E-book"]
---

{{< figure src="featured.png" >}}

{{< toc hide_on="xl" >}}

Um curso ministrado no Youtube para interessados a autores independentes que buscam desenvolver seus próprios livros.

## <i class="fas fa-bullhorn"></i>  Apresentação {id="apresentacao" title="Clique aqui!"}

Seja muito bem vindo ao Curso Quarto Livros. Temos um imenso prazer de repassar toda nossa experiência sobre desenvolvimento de livros. Neste curso, iremos apresentar uma das ferramentas que são utilizadas para o desenvolvimento de nossos materias. O Quarto é versátil para a criação não só de livros, mas muitos outros materiais. Aqui nos restingiremos apenas para a confecção de um livro. Bons estudos!



## O que você aprenderá

- Densenvolver um livro digital usando o [Quarto](https://quarto.org/).

## Visão geral do curso

Estamos focados em apresentar a forma básica de como desvolver um livro usando o Quarto. A partir do *Sweave*, *Markdown*, dentre outros, o desenvolvimento de materiais digitais foram se tornando cada vez mais acessíveis. Ferramentas impressionantes são desenvolvidas em R a todo momento. Dentre elas temos o Quarto, um sistema que integra diversas ferramentas para a renderização de um livros, em diversos formatos possíveis. Neste curso, daremos uma ideia do potencial que esta ferramenta pode ser utilizada para aqueles que buscam desenvolver seus próprios livros.

## <i class="fab fa-youtube"></i> Playlist do curso Estatística Experimental no YouTube {id="inscricao" title="Clique aqui!"}

- [Curso QLivros](https://www.youtube.com/playlist?list=PL-20Z1XFWKR0M7prT4svKG0qCoT7aU-EK)

<center>
<iframe width="560" height="315" src="https://www.youtube.com/embed/R0Z0tmTFXzY?si=nY_35MOZPUCX4K7r" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</center>





## <i class="fas fa-swatchbook"></i> Materiais de apoio {id="livro" title="Clique aqui!"}

- A disciplina de [Estatística e Probabilidade]({{< relref "/courses/epaec/" >}})
- Curso [EAR]({{< relref "/courses/ear/" >}})
- [Curso R]({{< relref "/courses/cursor/" >}})
- Livros: 
  - [R Básico](/books/eambr01/index.html) 
  - [R Básico: manual de soluções](books/eambr01sol/index.html)

## Módulos nesse curso

{{< list_children >}}

## Encontre seu instrutor

{{< mention "admin" >}}


{{< cta cta_text="Comece o curso" cta_link="intror" >}}
